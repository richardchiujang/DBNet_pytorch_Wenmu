...
from dbnmodels import build_model
...
def cv2imread(img_path): # 處理 windows cv2 讀取中文檔名的問題
...
def init_args():
    ...
    # parser.add_argument('--model_path', default=r'output/twcc/model_best.pth', type=str)
    parser.add_argument('--input_folder', default='input', type=str, help='img path for predict')
    parser.add_argument('--output_folder', default='output/inference', type=str, help='img path for output')
...
if __name__ == '__main__':
    ...
    for img_path in tqdm(get_file_list(args.input_folder, p_postfix=['.jpg','.JPG'])):
    # for img_path in (get_file_list(args.input_folder, p_postfix=['.jpg'])):    
        preds, boxes_list, score_list, t = model.predict(img_path, is_output_polygon=args.polygon, short_size=1024)
        # img = draw_bbox(cv2.imread(img_path)[:, :, ::-1], boxes_list)
        img = draw_bbox(cv2imread(img_path)[:, :, ::-1], boxes_list)
        if args.show:
            show_img(preds)
        ...
        output_path = os.path.join(args.output_folder, img_path.stem + '_result.jpg')     # _result
        pred_path = os.path.join(args.output_folder, img_path.stem + '_pred.jpg')
        # cv2.imwrite(output_path, img[:, :, ::-1]) # 會有中文問題
        # 这个函数返回一个元组，其中第一个元素是一个布尔值，表示编码是否成功，第二个元素是一个numpy数组，表示编码后的图像数据
        cv2.imencode('.jpg', img[:, :, ::-1])[1].tofile(output_path) # 正確方法
